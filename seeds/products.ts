export const products = [
  {
    productName: 'Apple Iphone 12 Pro Max | 256GB | Pacific Blue | 5G',
    productDescription: [
      '6.7-inch Super Retina XDR display',
      'Ceramic Shield, tougher than any smartphone glass',
      'A14 Bionic chip, the fastest chip ever in a smartphone',
      'Pro camera system with 12MP Ultra Wide, Wide and Telephoto cameras; 5x optical zoom range; Night mode, Deep Fusion, Smart HDR 3, Apple ProRAW, 4K Dolby Vision HDR recording',
      'LiDAR Scanner for improved AR experiences, Night mode portraits',
      '12MP TrueDepth front camera with Night mode, 4K Dolby Vision HDR recording',
      'Industry-leading IP68 water resistance',
      'Supports MagSafe accessories for easy attach and faster wireless charging',
      'iOS with redesigned widgets on the Home screen, all-new App Library, App Clips and more',
    ],
    productMRP: 1599.99,
    productImage: [
      'https://img.freepik.com/free-vector/realistic-smartphone-device_52683-29765.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/mobile-phone-different-perspectives_52683-51648.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/elegant-smartphone-composition_23-2149437101.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.7,
      numReviews: 11202,
    },
  },
  {
    productName:
      'MacBook Pro 13-inch | 1TB | 2.3GHz Quad-Core 10th-Generation Intel Core i7 Processor | Space Grey',
    productDescription: [
      'Tenth-generation quad-core Intel Core i7 processor',
      'Brilliant Retina display with True Tone technology',
      'Backlit Magic Keyboard',
      'Touch Bar and Touch ID',
      'Intel Iris Plus Graphics',
      'Ultrafast SSD',
      'Four Thunderbolt 3 (USB-C) ports',
      'Up to 10 hours of battery life',
      '802.11ac Wi-Fi',
      'Force Touch trackpad',
    ],
    productMRP: 2199.99,
    productImage: [
      'https://img.freepik.com/free-photo/electronic-device-balancing-concept_23-2150422322.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-psd/mock-up-macbookpro_1022-38.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/closeup-shot-open-laptop-dark-surface-work-from-home-concept_181624-61063.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.5,
      numReviews: 14892,
    },
  },
  {
    productName:
      'Apple Watch Series 6 | 44mm | GPS | Space Grey Aluminium Case',
    productDescription: [
      'Measure your blood oxygen with an all-new sensor and app',
      'Check your heart rhythm with the ECG app',
      'The Always-On Retina display is 2.5x brighter outdoors when your wrist is down',
      'S6 SiP is up to 20% faster than Series 5',
      '5GHz Wi-Fi and U1 Ultra Wideband chip',
      'Track your daily activity on Apple Watch and see your trends in the Fitness app on iPhone',
      'Measure workouts like running, walking, cycling, yoga, swimming and dance',
      'Swimproof design',
      'Sync your favourite music, podcasts and audiobooks',
    ],
    productMRP: 699.99,
    productImage: [
      'https://img.freepik.com/free-psd/smartwatch-mock-up-design_1135-61.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/smart-watch_23-2147519962.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/smartwatch-front-side_23-2147498802.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.9,
      numReviews: 7852,
    },
  },
  {
    productName: 'Apple AirPods Pro | White | Active Noise Cancellation',
    productDescription: [
      'Active Noise Cancellation for immersive sound',
      'Transparency mode for hearing and connecting with the world around you',
      'Three sizes of soft, tapered silicone tips for a customisable fit',
      'Sweat and water resistant',
      'Adaptive EQ automatically tunes music to the shape of your ear',
      'Easy setup for all your Apple devices',
      'The Wireless Charging Case delivers more than 24 hours of battery life',
    ],
    productMRP: 249.99,
    productImage: [
      'https://img.freepik.com/free-photo/earphones-with-case-surrounded-by-nature-scene_23-2150165594.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/portrait-expressive-young-woman-with-airpods_1258-48215.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/wireless-inear-headphones-with-case-pink-background-flat-lay_169016-23020.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.7,
      numReviews: 24865,
    },
  },
  {
    productName: 'Samsung Galaxy S21 Ultra 5G | 256GB | Phantom Black',
    productDescription: [
      '6.8-inch Dynamic AMOLED 2X display',
      'Exynos 2100 processor',
      '12GB RAM',
      '256GB internal storage',
      '108MP + 12MP + 10MP + 10MP rear camera',
      '40MP front camera',
      '5000mAh battery',
      'Android 11',
    ],
    productMRP: 1299.99,
    productImage: [
      'https://img.freepik.com/free-vector/realistic-phones-different-views_52683-28436.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-psd/smartphone-mockup_1310-817.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.2,
      numReviews: 6557,
    },
  },
  {
    productName: 'Samsung watch 4 | 44mm | Black | Bluetooth',
    productDescription: [
      '1.4-inch Super AMOLED display',
      'Exynos W920 processor',
      '1.5GB RAM',
      '16GB internal storage',
      '5ATM + IP68 + MIL-STD-810G',
      'Accelerometer, Barometer, Gyro Sensor, Light Sensor',
      '247mAh battery',
      'Wear OS',
    ],
    productMRP: 449.99,
    productImage: [
      'https://img.freepik.com/free-photo/smartwatch-screen-digital-device_53876-97295.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/realistic-style-fitness-trackers_23-2148509776.jpg?size=626&ext=jpgF',
    ],
    productrating: {
      rating: 4.5,
      numReviews: 7852,
    },
  },
  {
    productName:
      'Samsung Galaxy Buds Pro | Phantom Black | Active Noise Cancellation',
    productDescription: [
      'Active Noise Cancellation for immersive sound',
      'Transparency mode for hearing and connecting with the world around you',
      'Three sizes of soft, tapered silicone tips for a customisable fit',
      'Sweat and water resistant',
      'Adaptive EQ automatically tunes music to the shape of your ear',
      'Easy setup for all your devices',
      'The Wireless Charging Case delivers more than 24 hours of battery life',
    ],
    productMRP: 189.99,
    productImage: [
      'https://img.freepik.com/free-photo/black-wireless-earbud-with-charging-case_1268-14371.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 3.2,
      numReviews: 245,
    },
  },
  {
    productName:
      'Samsung Galaxy A51 | 6GB RAM and 128GB ROM | Prism Crush Black',
    productDescription: [
      '6.5-inch Super AMOLED display',
      'Exynos 9611 processor',
      '6GB RAM',
      '128GB internal storage',
      '48MP + 12MP + 5MP + 5MP rear camera',
      '32MP front camera',
      '4000mAh battery',
      'Android 10',
    ],
    productMRP: 349.99,
    productImage: [
      'https://img.freepik.com/free-photo/three-isolated-smartphone-blue-background_125540-788.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/isolated-phone-grey-background_125540-777.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.1,
      numReviews: 7852,
    },
  },
  // Now we are adding clothes products(index 8)
  {
    productName: "Men's Regular Fit Shirt | Blue | Pure Cotton",
    productDescription: [
      '100% Cotton',
      'Regular fit',
      'Long sleeve',
      'Button down collar',
      'Machine wash',
    ],
    productMRP: 29.99,
    productImage: [
      'https://img.freepik.com/free-photo/smiling-man-wearing-sunglasses_171337-14990.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/beautiful-men-fashion-shirt_74190-5185.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/beautiful-men-fashion-shirt_1203-7644.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 3.9,
      numReviews: 4752,
    },
  },
  {
    productName: "Mens' Regulat Fit T-Shirt | Black | Pure Cotton",
    productDescription: [
      '100% Cotton',
      'Regular fit',
      'Short sleeve',
      'Round neck',
      'Machine wash',
    ],
    productMRP: 19.99,
    productImage: [
      'https://img.freepik.com/free-photo/blank-black-t-shirt-hanger-isolated-white-space_74952-876.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-psd/simple-black-men-s-tee-mockup_53876-57893.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.2,
      numReviews: 3547,
    },
  },
  {
    productName: "Men's Denim Jeans | Blue | Pure Cotton",
    productDescription: [
      '100% Cotton',
      'Regular fit',
      'Long pant',
      'Button down',
      'Machine wash',
    ],
    productMRP: 39.99,
    productImage: [
      'https://img.freepik.com/free-photo/jeans_1203-8094.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/blue-jeans-fabric-details_150588-40.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.5,
      numReviews: 14236,
    },
  },
  // Now we are adding shoes products(index 11)
  {
    productName: "Beta men's running shoes | Black",
    productDescription: [
      'Synthetic sole',
      'Shaft measures approximately low-top from arch',
      'Nike React foam at the midsole is lightweight, springy and durable',
      'An Air Zoom unit at the forefoot is twice as large as previous versions, delivering more bounce with every step',
      `It's top-loaded to be closer to your foot for better responsiveness`,
      'A midfoot band adapts to the size of your foot, keeping you comfortable and secure with every step',
      'The higher heel with foam on the sides creates a cupping sensation and adds stability',
    ],
    productMRP: 129.99,
    productImage: [
      'https://img.freepik.com/free-photo/sport-shoes-running_1203-7549.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/close-up-male-s-feet-forest_23-2148776366.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.5,
      numReviews: 1489,
    },
  },
  {
    productName: "Apha Men's Sneakers | White",
    productDescription: [
      'Rubber sole',
      'Shaft measures approximately low-top from arch',
      'Platform measures approximately 0-3 inches',
      'Boot opening measures approximately 0-3 inches around',
      'Regular fit',
      'Lace closure',
      'Leather upper',
      'Rubber outsole',
    ],
    productMRP: 99.99,
    productImage: [
      'https://img.freepik.com/free-photo/white-high-top-sneakers-unisex-footwear-fashion_53876-106036.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/denim-sneakers-street-style_53876-146049.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/white-training-sneakers-unisex-sportswear-fashion-shoot_53876-102187.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.2,
      numReviews: 3547,
    },
  },
  {
    productName: 'Gamma Men’s Running Shoes | Blue',
    productDescription: [
      'Rubber sole',
      'Shaft measures approximately low-top from arch',
      'Platform measures approximately 0-3 inches',

      'Boot opening measures approximately 0-3 inches around',
      'Regular fit',
      'Lace closure',
      'Textile upper',
      'Rubber outsole',
    ],
    productMRP: 89.99,
    productImage: [
      'https://img.freepik.com/free-photo/pair-trainers_144627-3800.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/pair-trainers_144627-3809.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.5,
      numReviews: 1489,
    },
  },
  // Now we are adding Home and kitchen products(index 14)
  {
    productName: 'KitchenAid KSM150PSER Artisan Tilt-Head Stand Mixer | White',
    productDescription: [
      '10-speed slide control ranges from a very fast whip to a very slow stir',
      'Includes Flat Beater, Dough Hook and Wire Whip',
      'Power Hub for Additional Attachments',
      'Unique Mixing Action',
      'Assembled with Pride in Greenville, Ohio',
    ],
    productMRP: 99.99,
    productImage: [
      'https://img.freepik.com/free-photo/woman-enjoying-her-juice-recipe_23-2149257488.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/fitness-woman-preparing-detox-juice_23-2148207459.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 3.8,
      numReviews: 142,
    },
  },
  {
    productName: 'Cuisinart TOA-60 Convection Toaster Oven Airfryer | Silver',
    productDescription: [
      '1800-Watts with 7 functions: air fry, convection bake, convection broil, bake, broil, Warm, toast',
      '0. 6 cubic foot non-stick interior with oven light. Dimensions (L x W x H): 15. 50 x 16. 00 x 14. 00 inches',
      'Capacity: toast 6 slices of bread, air fry 3lbs of chicken wings, bake a 12-inch pizza or roast a 4 pound chicken',
      'Adjustable thermostat with 60-minute timer/auto shutoff, toast shade selector-timer.UC Cubic Feet: 3.249 lb',
      'Includes: oven rack, baking pan, air fryer rack/basket. Mc cubic feet: 3. 390',
      '60-minute timer with auto shutoff',
      'Includes: oven rack, baking pan, air fryer rack/basket',
    ],
    productMRP: 49.99,
    productImage: [
      'https://img.freepik.com/free-photo/mother-with-daughter-kitchen_1157-18657.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/happy-black-family-checking-toast_23-2148947197.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.4,
      numReviews: 789,
    },
  },
  // Now we are adding Electronics products(index 16)
  {
    productName:
      'Godrej 190 L 5 Star Inverter Direct-Cool Single Door Refrigerator | Silver',
    productDescription: [
      'Direct Cool, Single Door: Economical requires manual defrosting',
      'Capacity 190 L: Suitable for families with 2-3 members',
      'Warranty: 1 year on product, 10 years on compressor',
      'Energy Rating: 5 Star',
      'Shelf Type: Toughened Glass',
      'Also included in the box: User manual, Warranty card',
    ],
    productMRP: 249.99,
    productImage: [
      'https://img.freepik.com/free-vector/refrigerator-with-closed-opened-door_1308-95206.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/refrigerator-with-lots-food_1308-94526.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.4,
      numReviews: 789,
    },
  },
  {
    productName:
      'Samsung 6.5 kg Fully-Automatic Top Loading Washing Machine | White',
    productDescription: [
      'Fully-automatic top load washing machine: Affordable with great wash quality, Easy to use',
      'Capacity 6.5 kg: Suitable for families with 3 to 4 members',
      'Warranty: 2 years on product, 10 years on motor',
      'Design - door, color - PM grey, panel display - red LED',
      'Feature - air turbo, auto restart, child lock, magic filter, tempered glass window, drum type - diamond drum, pulsator- center jet, water level - 5 levels',
      'Cycle - blanket, delicates, eco tub clean, additional cycle - normal, quick wash, soak plus normal, number of cycle - 6',
      'Physical specification - voltage/frequency- 220/50, drum material- SS',
      'Dimensions - 540 x 560 x 906 mm',
      'Powerful filtration and easy cleaning',
    ],
    productMRP: 249.99,
    productImage: [
      'https://img.freepik.com/free-vector/washing-machine-with-front-door_1308-74400.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/washing-machine-realistic-background-with-laundry-equipment-symbols-illustration_1284-29130.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.6,
      numReviews: 7789,
    },
  },
  {
    productName: 'LG 260 L 3 Star Frost Free Double Door Refrigerator | Silver',
    productDescription: [
      'Frost Free Refrigerator: Auto defrost function to prevent ice-build up',
      'Capacity 260 L: Suitable for families with 2-3 members',
      'Warranty: 1 year on product, 10 years on compressor',
      'Energy Rating: 3 Star',
      'Shelf Type: Toughened Glass',
      'Also included in the box: User manual, Warranty card',
    ],
    productMRP: 449.99,
    productImage: [
      'https://img.freepik.com/free-vector/refrigerator-closed-opened-door-with-lots-food_1308-103210.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/refrigerator-with-lots-food_1308-91891.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.2,
      numReviews: 1789,
    },
  },
  {
    productName: 'Havells 15 L Storage Water Geyser | White',
    productDescription: [
      'Capacity: 15 Litres; Wattage: 2000 Watts',
      'Incoloy coated heating element',
      'Heavy duty anode rod protects tank from corrosion',
      'Adjustable knob for temperature settings between 25 °C to 75°C to get the desired temperature',
      'Product Dimensions: 37.3 cms X 37.3 cms X 34.8 cms',
      'Warranty: 5 years on inner container, 2 years comprehensive',
      'Included in the box: 1 Vertical Storage Water Heater, User Manual, Mounting Screws',
    ],
    productMRP: 149.99,
    productImage: [
      'https://img.freepik.com/free-vector/colored-realistic-electric-water-heater-boiler-illustration-with-big-white-headline-light-blue-wall_1284-33272.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/colored-realistic-water-heater-boiler-composition-with-two-isolated-different-equipment-pipes_1284-33271.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.7,
      numReviews: 1100,
    },
  },
  // Now we are adding Furniture products(index 20)
  {
    productName: 'FurnitureKraft Sofa | Sky Blue',
    productDescription: [
      'Product Dimensions: Length (75 inches), Width (32 inches), Height (32 inches)',
      'Primary Material: Wood, Upholstery Material: Leatherette',
      'Color: Sky Blue, Style: Modern',
      'Seating Capacity: Three seat',
      'Assembly Required: The product requires carpenter assembly and will be provided by the seller',
      'Warranty: 1 year on product',
    ],
    productMRP: 349.99,
    productImage: [
      'https://img.freepik.com/free-psd/wall-mockup-psd-with-blue-sofa-living-room_53876-144931.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/chic-modern-luxury-aesthetics-style-living-room-blue-tone_53876-125839.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.9,
      numReviews: 689,
    },
  },
  {
    productName: 'FurnitureKraft Dining Table | Brown',
    productDescription: [
      'Product Dimensions: Length (75 inches), Width (32 inches), Height (32 inches)',
      'Primary Material: Wood',
      'Color: Brown, Style: Modern',
      'Seating Capacity: 6 seat',
      'Warranty: 6 months on product',
    ],
    productMRP: 139.99,
    productImage: [
      'https://img.freepik.com/free-vector/top-view-empty-restaurant-table-with-realistic-design_23-2147914129.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/dining-room-interior-design-with-furniture_1308-67305.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/loft-authentic-dining-room-interior-design-with-picture-frame_53876-111114.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.5,
      numReviews: 789,
    },
  },
  {
    productName: 'FurnitureKraft Bed | Brown & White',
    productDescription: [
      'Product Dimensions: Length (75 inches), Width (48 inches), Height (20 inches)',
      'Primary Material: Wood',
      'Color: Brown & White, Style: Modern',
      'Warranty: 6 months on product',
    ],
    productMRP: 109.99,
    productImage: [
      'https://img.freepik.com/free-photo/3d-rendering-beautiful-comtemporary-luxury-bedroom-suite-hotel-with-tv_105762-2064.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-psd/classic-bedroom-hotel-room-with-double-bed_176382-88.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-psd/realistic-modern-double-bedroom-with-furniture-frame_176382-411.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.0,
      numReviews: 351,
    },
  },
  {
    productName: 'FurnitureKraft Wardrobe | Brown',
    productDescription: [
      'Product Dimensions: Length (80 inches), Width (22 inches), Height (75 inches)',
      'Primary Material: Wood',
      'Color: Brown, Style: Modern',
      'Warranty: 9 months on product',
      'Assembly Required: The product requires carpenter assembly and will be provided by the seller',
    ],
    productMRP: 120.99,
    productImage: [
      'https://img.freepik.com/free-vector/vector-brown-cloakroom-closet-with-shelves-full-boxes-clothes-shirts-boots-shoes-hats-front-view_1284-48002.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/3d-rendering-luxury-scandinavian-wood-walk-closet-with-wardrobe-make-up-table_105762-2147.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 5.0,
      numReviews: 220,
    },
  },
  // Now we are adding Grocery products(index 24)
  {
    productName: 'GroceryKart Wheat Flour | 1kg',
    productDescription: [
      'Wheat Flour',
      '100% Whole Wheat',
      'No Maida',
      'No Preservatives',
      'No Additives',
      'No Bleaching',
    ],
    productMRP: 1.0,
    productImage: [
      'https://img.freepik.com/free-photo/ingredient-bags-full-flour_23-2149482551.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/ingredient-bags-full-flour_23-2149482544.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/ingredient-bags-full-flour_23-2149482548.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.5,
      numReviews: 18570,
    },
  },
  {
    productName: 'GroceryKart Rice | 1kg',
    productDescription: [
      'Rice',
      '100% Whole Rice',
      'No Preservatives',
      'No Additives',
      'No Bleaching',
    ],
    productMRP: 2.0,
    productImage: [
      'https://img.freepik.com/free-photo/top-view-raw-rice-inside-plate-dark-desk_179666-27235.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/top-view-raw-rice-inside-brown-plate-grey-surface_140725-90606.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.8,
      numReviews: 25570,
    },
  },
  {
    productName: 'GroceryKart Sugar | 1kg',
    productDescription: [
      'Sugar',
      '100% Whole Sugar',
      'No Preservatives',
      'No Additives',
      'No Bleaching',
    ],
    productMRP: 0.79,
    productImage: [
      'https://img.freepik.com/free-photo/world-diabetes-day-sugar-wooden-bowl-dark-surface_1150-26666.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/world-diabetes-day-sugar-wooden-bowl-dark-background_1150-26667.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/sugar-lies-black-background_1304-3567.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.4,
      numReviews: 12001,
    },
  },
  {
    productName: 'GroceryKart Salt | 1kg',
    productDescription: [
      'Salt',
      'No Preservatives',
      'No Additives',
      'No Bleaching',
    ],
    productMRP: 0.49,
    productImage: [
      'https://img.freepik.com/free-vector/glass-salt-shaker-pile-salt_1441-4176.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/realistic-pack-templates-different-shape-size-grey-background_1284-7391.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 3.7,
      numReviews: 27465,
    },
  },
  {
    productName: 'GroceryKart Tea | 1kg',
    productDescription: [
      'Tea',
      'Special Darjeeling Tea',
      'No Preservatives',
      'No Additives',
      'No Bleaching',
    ],
    productMRP: 3.99,
    productImage: [
      'https://img.freepik.com/free-vector/matcha-green-tea-powder-vector-realistic-product-placement-mock-up-healthy-drink-label-designs_1268-18117.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/cup-tea-ad_52683-35124.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.9,
      numReviews: 12001,
    },
  },
  // Now we are adding beauty products(index 29)
  {
    productName: 'BeautyKart Lipstick | 1pc',
    productDescription: [
      'Lipstick',
      '100% Organic',
      'Dermatologically Tested',
      'Deep Moisturizing',
    ],
    productMRP: 3.49,
    productImage: [
      'https://img.freepik.com/free-photo/top-view-lipstick-with-brush_23-2149096695.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-photo/portrait-young-girl-red-beret-painting-her-lips-with-bright-lipstick-pink-background_197531-17567.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.8,
      numReviews: 671,
    },
  },
  {
    productName: 'BeautyKart Face Cream | 1pc',
    productDescription: [
      'Face Cream',
      '100% Organic',
      'Dermatologically Tested',
      'Deep Moisturizing',
      'No Parabens and Sulphates',
    ],
    productMRP: 3.49,
    productImage: [
      'https://img.freepik.com/free-vector/skin-treatment-cosmetic_23-2147697213.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/cosmetic-cream-packaging_1268-1265.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.3,
      numReviews: 6742,
    },
  },
  {
    productName: 'BeautyKart Face Wash | 1pc',
    productDescription: [
      'Face Wash',
      '100% Organic',
      'Dermatologically Tested',
      'Deep Moisturizing',
      'No Parabens and Sulphates',
      'No Bleaching',
    ],
    productMRP: 3.49,
    productImage: [
      'https://img.freepik.com/free-psd/cosmetic-packaging-mock-up_1307-657.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/blue-cosmetic-background_23-2147677031.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 4.6,
      numReviews: 3451,
    },
  },
  {
    productName: 'BeautyKart Shampoo | 1pc',
    productDescription: [
      'Shampoo',
      '100% Organic',
      'Dermatologically Tested',
      'Deep Moisturizing',
      'No Parabens and Sulphates',
      'Anti-Dandruff',
    ],
    productMRP: 3.49,
    productImage: [
      'https://img.freepik.com/free-vector/realistic-banner-aloe-vera-products_23-2147640895.jpg?size=626&ext=jpg',
      'https://img.freepik.com/free-vector/solar-protection-bottles-products-summer_18591-52635.jpg?size=626&ext=jpg',
    ],
    productrating: {
      rating: 3.8,
      numReviews: 786,
    },
  },
];
